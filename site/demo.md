...
# BMI160 SPI Sensor Emulator Demo

<p class="lead">
This hardware-in-the-loop demo physically replaces a BMI160 accelerometer with the RH01T9k emulator while the ESP32 runs the same firmware.
</p>
...
The ESP32 first reads acceleration data from a physical BMI160 over SPI and displays X/Y/Z values on an OLED.

The BMI160 is then replaced by the RH01T9k. The ESP32 firmware remains unchanged and continues issuing the same SPI transactions, while RH01T9k returns simulated BMI160 register data configured from Python.
...
The X/Y/Z accelerometer values include the effect of gravity. At 0°, the sensor reports approximately X = 0 g, Y = 0 g, Z = -1 g. At 90°, it reports approximately X = +1 g, Y = 0 g, Z = 0 g.

## ESP32 firmware

The ESP32 firmware used in this demo is available on GitHub:

[ESP32 BMI160 SPI firmware](https://github.com/mildmidwesterner/esp32-spi-bmi160)

The same firmware is used with both the physical BMI160 and the RH01T9k emulator.

## Python emulator script

The following script configures RH01T9k as a BMI160-compatible SPI peripheral and generates a repeatable tilt sequence.

```python
"""
BMI160 emulator - scripted motion demo.

The SPI slave emulator synthesizes a
motion sequence:
